...
class PDFParser:
    ...
    def parse(self, pdf_path: str | Path, password: str | None = None) -> ParsedDocument:
        ...
        if self.config.merge_adjacent_text_blocks:
            self._merge_adjacent_blocks(pages)
    ...
    def _extract_pages(self, document: pymupdf.Document) -> list[PageData]:
        ...
        for page_index in range(document.page_count):
            ...
            blocks, removed_noise_count = self._extract_text_blocks(
                page=page,
    ...
    def _extract_text_blocks(
        ...
        for raw_block in page_dict.get("blocks", []):
            ...
            text = clean_code_text(code_lines) if monospace else merge_extracted_lines(normalized_lines)
            text = clean_extracted_text(text) if not monospace else text
        ...
        return _sort_blocks(blocks), removed_noise_count
    ...
    def _classify_page_layouts(self, pages: Sequence[PageData]) -> None:
        for page in pages:
            ...
            distinct_x_bands = len({round(center / max(page.width, 1.0) * 5) for center in centers})

            if (
                short_ratio >= self.config.complex_layout_short_block_ratio
                and x_spread >= self.config.complex_layout_min_x_spread_ratio
                and median_chars <= self.config.complex_layout_short_block_chars
                and distinct_x_bands >= 3
            ):
                page.layout_kind = "complex"
            else:
                page.layout_kind = "normal"
    ...
    def _merge_adjacent_blocks(self, pages: Sequence[PageData]) -> None:
        for page in pages:
    ...
    def _should_merge_blocks(self, left: TextBlock, right: TextBlock) -> bool:
...
def _infer_heading(
    ...
    sentence_like = _looks_like_sentence_or_list_item(text)

    if page.layout_kind == "complex":
        if config.complex_layout_strategy == "skip":
            return None, None
        # 图解/幻灯片页面只允许一个视觉上非常明确的顶部大标题。
...
def _looks_like_sentence_or_list_item(text: str) -> bool:
    compact_length = visible_char_count(text)
    if re.search(r"[。！？!?；;]$", text):
        return True
    ...
    if ":" in text or "：" in text:
        ...
        if visible_char_count(tail) >= 16:
            return True
    if compact_length > 48 and re.search(r"(?:用于|负责|通过|需要|包括|可以|进行|完成)", text):
        return True
    return False
