...
class ParserConfig:
    ...
    remove_promotional_contacts: bool = True
    reconstruct_code_blocks: bool = True
    code_indent_spaces: int = 4
    code_min_marker_lines: int = 2
    revalidate_headings_in_structure: bool = True
    keep_short_complex_layout_labels: bool = True

    detect_complex_layout: bool = True
    complex_layout_strategy: ComplexLayoutStrategy = "conservative"
    complex_layout_min_blocks: int = 5
    complex_layout_short_block_chars: int = 24
    complex_layout_short_block_ratio: float = 0.50
    complex_layout_min_x_spread_ratio: float = 0.28
    detect_toc_pages: bool = True
    toc_min_numbered_entries: int = 6
    toc_max_average_entry_chars: int = 42
    index_toc_pages: bool = False

    heading_max_chars: int = 80
    single_level_heading_max_chars: int = 38
    heading_min_font_ratio: float = 1.12
    heading_bold_min_font_ratio: float = 1.04
    ...
    def validate(self) -> None:
        ...
        if self.code_indent_spaces not in {2, 4, 8}:
            raise ValueError("code_indent_spaces 只能是 2、4 或 8")
        if self.code_min_marker_lines < 1:
            raise ValueError("code_min_marker_lines 不能小于 1")
        ...
        if self.toc_min_numbered_entries < 3:
            raise ValueError("toc_min_numbered_entries 不能小于 3")
        if self.toc_max_average_entry_chars < 8:
            raise ValueError("toc_max_average_entry_chars 过小")
...
class QualityConfig:
    ...
    max_malformed_code_chunk_ratio: float = 0.15
    max_python_ast_failure_ratio: float = 0.35
    max_code_indentation_issue_ratio: float = 0.20
    max_split_identifier_candidate_ratio: float = 0.10
    max_function_boundary_violation_ratio: float = 0.10
    max_glued_list_paragraph_ratio: float = 0.03
    ...
    def validate(self) -> None:
        ratio_fields = {
            ...
            "max_malformed_code_chunk_ratio": self.max_malformed_code_chunk_ratio,
            "max_python_ast_failure_ratio": self.max_python_ast_failure_ratio,
            "max_code_indentation_issue_ratio": self.max_code_indentation_issue_ratio,
            "max_split_identifier_candidate_ratio": self.max_split_identifier_candidate_ratio,
            "max_function_boundary_violation_ratio": self.max_function_boundary_violation_ratio,
            "max_glued_list_paragraph_ratio": self.max_glued_list_paragraph_ratio,
        }
