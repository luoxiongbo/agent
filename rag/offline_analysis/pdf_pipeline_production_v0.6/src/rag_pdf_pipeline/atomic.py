...
class AtomicUnitBuilder:
    ...
    def build(self, document: ParsedDocument) -> list[Section]:
        ...
        def flush_section(end_page: int) -> None:
            ...
            page_roles = {str(unit.metadata.get("page_role", "body")) for unit in units}
            index_enabled = any(bool(unit.metadata.get("index_enabled", True)) for unit in units)
            record_type = "toc" if page_roles == {"toc"} else "body"
            sections.append(
                Section(
        ...
        for page in document.pages:
            ...
            if (
                ...
                and (
                    previous_page.layout_kind in {"complex", "sparse"}
                    or previous_page.page_role in {"toc", "cover"}
                )
            ):
                ...
                if kind == "table":
                    current_units.append(
                        self._table_unit(
                            ...
                            index_enabled=page.index_enabled,
                        )
                    )
                else:
    ...
    def _text_units(
        ...
        if kind == "code":
            code_analysis = analyze_python_code(text)
            metadata["code_language"] = "python" if code_analysis.is_python else "unknown"
            metadata["python_ast_parseable"] = code_analysis.ast_parseable
            metadata["python_syntax_error"] = code_analysis.syntax_error
            metadata["code_indentation_issue_count"] = code_analysis.indentation_issue_count
            metadata["split_identifier_candidate_count"] = (
                code_analysis.split_identifier_candidate_count
            )
            metadata["function_boundary_violation_count"] = (
                code_analysis.function_boundary_violation_count
            )
    ...
    def _classify(
        ...
        # 代码优先于复杂版面短标签；否则代码页会被错误归类为 layout。
    ...
    def _should_merge_paragraph_units(
        ...
        lexical_word_break = is_probable_cjk_word_break(current.text, next_unit.text)
        if is_paragraph_starter(next_unit.text) and not lexical_word_break:
            return False
        ...
        if current.page_end == next_unit.page_start:
            ...
            if -3 <= vertical_gap <= max_gap and x_offset <= self.config.max_block_x_offset:
                return True
            return lexical_word_break and x_offset <= self.config.max_block_x_offset * 1.5
        ...
        return (
            current_near_bottom
            and next_near_top
            and x_offset <= self.config.max_block_x_offset
            and (lexical_word_break or starts_like_continuation(next_unit.text))
        )
            ...
            if (
                ...
                result[-1] = replace(
                    ...
                    metadata={
                        **previous.metadata,
                        "page_roles": sorted(
                            set(previous.metadata.get("page_roles", []))
                            | set(section.metadata.get("page_roles", []))
                        ),
                        "index_enabled": bool(previous.metadata.get("index_enabled", True))
                        or bool(section.metadata.get("index_enabled", True)),
                    },
                )
            else:
