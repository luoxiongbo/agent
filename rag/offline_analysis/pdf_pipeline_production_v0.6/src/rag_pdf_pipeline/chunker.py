...
class SmartParentChildChunker:
    ...
    def _expand_oversized_units(
        ...
        for unit in section.units:
            ...
            should_structure_split_code = (
                unit.kind == "code"
                and looks_like_python_code(unit.text)
                and (
                    unit_tokens > body_target
                    or len(split_python_top_level_blocks(unit.text)) > 1
                )
            )
            if unit_tokens <= body_max and not should_structure_split_code:
                result.append(unit)
                continue
    ...
    def _split_code(
        ...
        if looks_like_python_code(unit.text):
            logical_blocks = split_python_top_level_blocks(unit.text)
        else:
            logical_blocks = [unit.text]
        ...
        for block in logical_blocks:
            ...
            if block_tokens > max_tokens:
                if current:
                    groups.append("\n\n".join(current))
                    current = []
                groups.extend(self._split_oversized_code_block(block, max_tokens))
                continue

            candidate = "\n\n".join([*current, block])
            begins_definition = bool(
                re.match(r"^\s*(?:@|async\s+def|def|class)\b", block)
            )
            current_has_definition = any(
                re.search(r"(?:^|\n)\s*(?:async\s+def|def|class)\b", item)
                for item in current
            )
            if current and (
                estimate_tokens(candidate) > target_tokens
                or (begins_definition and current_has_definition)
            ):
                groups.append("\n\n".join(current))
                current = [block]
            else:
        ...
        return [
            self._piece(
                ...
                total=total,
                extra_metadata={
                    "split_strategy": "code_top_level",
                    "code_boundary": "function_or_class",
                    "force_child_boundary_before": index > 0,
                    "force_child_boundary_after": total > 1,
                    "function_signature": first_definition_signature(group),
                },
            )
            for index, group in enumerate(groups)
        ]

    def _split_oversized_code_block(
        self,
        block: str,
        max_tokens: int,
    ) -> list[str]:
        signature = first_definition_signature(block)
        lines = block.splitlines()
        if not signature or len(lines) <= 2:
            return hard_split_text(block, max_tokens)

        signature_index = next(
            index for index, line in enumerate(lines) if line.rstrip() == signature.rstrip()
        )
        prefix_lines = lines[: signature_index + 1]
        body_lines = lines[signature_index + 1 :]
        continuation = "    # PDF 代码续段：保留函数签名以提供检索上下文"
        prefix = "\n".join(prefix_lines)
        budget = max(30, max_tokens - estimate_tokens(prefix) - estimate_tokens(continuation))
        ...
        for index, group in enumerate(body_groups):
            header = prefix if index == 0 else prefix + "\n" + continuation
            piece = header + "\n" + "\n".join(group)
            if estimate_tokens(piece) <= max_tokens:
                pieces.append(piece)
            else:
                raw_groups = hard_split_text("\n".join(group), budget)
    ...
    def _chunk_section(
        ...
        current_segment: list[AtomicUnit] = []
        for unit in units:
            if unit.metadata.get("force_child_boundary_before") and current_segment:
                segments.append(current_segment)
                current_segment = []
            current_segment.append(unit)
            if unit.metadata.get("force_child_boundary_after"):
                segments.append(current_segment)
                current_segment = []
        if current_segment:
            segments.append(current_segment)

        primary_groups: list[list[AtomicUnit]] = []
        for segment in segments:
            vectors = self.encoder.encode([unit.text for unit in segment])
            similarities = adjacent_similarities(vectors)
            token_counts = [estimate_tokens(unit.text) for unit in segment]
            primary_groups.extend(
                _semantic_partition(
                    ...
                    semantic_weight=self.config.semantic_weight,
                    length_weight=self.config.length_weight,
                    group_ids=[unit.group_id for unit in segment],
