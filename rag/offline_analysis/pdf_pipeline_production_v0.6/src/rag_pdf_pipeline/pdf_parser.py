...
class PDFParser:
    ...
    def _extract_tables(
        ...
        for index, table in enumerate(getattr(finder, "tables", []), start=1):
            ...
            if not rows:
                rejected += 1
                rejection_reasons["empty_rows"] += 1
                continue
            ...
            if score < self.config.min_table_quality_score:
                ...
                rejected += 1
                for reason in reasons or ["score_below_threshold"]:
        ...
        candidate_count = len(getattr(finder, "tables", []))
        if candidate_count:
            LOGGER.info(
                "第 %d 页表格候选汇总：候选=%d，接受=%d，拒绝=%d，主要原因=%s",
                page_number,
                candidate_count,
                len(result),
                rejected,
                rejection_reasons.most_common(3),
            )
        return result, rejected
    ...
    def _classify_page_layouts(self, pages: Sequence[PageData]) -> None:
        for page in pages:
            ...
            monospace_chars = sum(
                visible_char_count(block.text) for block in blocks if block.monospace
            )
            monospace_block_count = sum(block.monospace for block in blocks)
            if (
                monospace_block_count >= 3
                and monospace_chars / max(total_chars, 1) >= 0.55
            ):
                # 代码页通常由许多短行组成，不能按幻灯片/流程图复杂页处理。
                page.layout_kind = "normal"
                self._classify_page_role(page)
                continue
    ...
    def _merge_adjacent_blocks(
        ...
        for page in pages:
            if page.layout_kind == "complex":
                # 复杂页面不做普通段落合并，但等宽代码仍必须按坐标合并。
                merged_code: list[TextBlock] = []
                for block in _sort_blocks(page.blocks):
                    if merged_code and self._should_merge_code_blocks(merged_code[-1], block):
                        merged_code[-1] = _merge_code_blocks(
                            merged_code[-1],
                            block,
                            indent_spaces=self.config.code_indent_spaces,
                        )
                    else:
                        merged_code.append(block)
                page.blocks = merged_code
                continue
