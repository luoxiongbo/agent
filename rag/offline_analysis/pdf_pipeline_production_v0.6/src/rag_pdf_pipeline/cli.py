...
from .runtime import PACKAGE_VERSION
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--index-toc-pages",
        action="store_true",
        help="允许目录/导航页进入向量索引；默认只保留但不索引",
    )
    parser.add_argument(
        "--keep-promotional-contacts",
        action="store_true",
        help="保留课程推广、微信号、扫码关注等宣传联系方式",
    )
    parser.add_argument(
        "--no-code-reconstruction",
        action="store_true",
        help="禁用基于行坐标的代码换行和缩进恢复",
    )
...
def load_config(args: argparse.Namespace) -> PipelineConfig:
    ...
    if args.index_toc_pages:
        config.parser.index_toc_pages = True
    if args.keep_promotional_contacts:
        config.parser.remove_promotional_contacts = False
    if args.no_code_reconstruction:
        config.parser.reconstruct_code_blocks = False
