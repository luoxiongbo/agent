...
def reconstruct_code_from_lines(
    ...
    for x0, raw in pairs:
        ...
        for fragment_index, fragment in enumerate(fragments):
            level = max(0, round((x0 - min_x) / indent_width))
            if fragment_index > 0 and previous.rstrip().endswith(":"):
                level = max(level, 1)
            # PDF 有时丢失缩进坐标，使用 Python 语法做最低限度的恢复。
            if previous.rstrip().endswith(":") and level == 0:
                level = 1
            if re.match(r"^(?:elif|else|except|finally)\b", fragment):
                level = max(0, level - 1)
